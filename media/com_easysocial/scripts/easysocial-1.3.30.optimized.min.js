...
}}}):b.startMigration()},"{initiateButton} click":function(){b.showCustomFields()},"{viewLog} click":function(){b.results().toggle()}}}),b.resolve()})}),EasySocial.module("admin/points/discover",function(){var b=this;EasySocial.require().script("progress/progress").language("COM_EASYSOCIAL_SCAN_COMPLETED").done(function(a){EasySocial.Controller("Points.Discover",{defaultOptions:{files:[],progressController:null,"{startButton}":"[data-points-discovery-start]","{progressBar}":".discoverProgress","{results}":"[data-points-discovery-result]"}},function(b){return{init:function(){b.initProgressBar()
...
EasySocial.Controller("Fields",{defaultOptions:{id:0,group:null,"{wrap}":"[data-fields-wrap]","{browser}":"[data-fields-browser]","{editor}":"[data-fields-editor]","{steps}":"[data-fields-steps]","{config}":"[data-fields-config]","{saveForm}":"[data-fields-save]",view:{config:"admin/profiles/form.fields.config"}}},function(b){return{init:function(){c=b,b.options.id=b.element.data("id"),d=b.addPlugin("browser"),e=b.addPlugin("editor"),f=b.addPlugin("steps");var g=[d.state,e.state,f.state];a.when.apply(null,g).done(function(){c.trigger("controllersReady")
...
b.replaceWith(d)}),this.resolve()}),EasySocial.module("oauth/facebook",function(a){var b=this;a(document).on("click","[data-oauth-facebook-login]",function(){var b=a(this),c=b.parents("[data-oauth-facebook]"),d="EasySocial.Controller.OAuth.Facebook";c.length<1||c.hasController(d)||c.addController(d,{url:b.data("oauth-facebook-url"),appId:b.data("oauth-facebook-appid")}).openDialog()}),EasySocial.Controller("OAuth.Facebook",{defaultOptions:{appId:null,url:"","{login}":"[data-oauth-facebook-login]","{revoke}":"[data-oauth-facebook-revoke]","{pushInput}":"[data-oauth-facebook-push]"}},function(b){return{init:function(){},openDialog:function(){var a=b.options.url,c=screen.width/2-150,d=screen.height/2-150;
window.open(a,"","scrollbars=no,resizable=no,width=300,height=300,left="+c+",top="+d)},"{pushInput} change":function(c){var d=a(c).val();1==d&&b.options.requestPush&&b.openDialog(b.options.addPublishURL),0==d&&b.openDialog(b.options.revokePublishURL)},"{login} click":function(){b.openDialog()},"{revoke} click":function(){var a=b.element.data("callback");EasySocial.dialog({content:EasySocial.ajax("site/views/oauth/confirmRevoke",{client:"facebook",callbackUrl:a})})}}}),b.resolve()}),EasySocial.module("groups/suggest",function(){var b=this;
EasySocial.require().view("site/groups/suggest.item").library("textboxlist").done(function(a){EasySocial.Controller("Groups.Suggest",{defaultOptions:{max:null,exclusive:!0,exclusion:[],minLength:1,highlight:!0,name:"uid[]",type:"",view:{suggestItem:"site/groups/suggest.item"}}},function(b){return{init:function(){var a=b.options;b.element.textboxlist({component:"es",name:a.name,max:a.max,plugin:{autocomplete:{exclusive:a.exclusive,minLength:a.minLength,highlight:a.highlight,showLoadingHint:!0,showEmptyHint:!0,query:function(b){var c=EasySocial.ajax("site/controllers/groups/suggest",{search:b,exclusion:a.exclusion});
return c}}}}).textboxlist("enable")},"{self} filterItem":function(a,c,d){var e=b.view.suggestItem(!0,{item:d,name:b.options.name});return d.menuHtml=e,d.html=e,d},"{self} filterMenu":function(c,d,e,f,g,h){if(!b.options.friendList){var i=h.getAddedItems(),j=a.pluck(i,"id"),j=j.concat(b.options.exclusion);f.each(function(){var b=a(this),c=b.data("item");b.toggleClass("hidden",a.inArray(c.id.toString(),j)>-1)})}}}}),b.resolve()})}),EasySocial.module("locations",function(a){var b=this;EasySocial.require().library("gmaps").done(),EasySocial.require().library("scrollTo").view("site/location/story.suggestion").language("COM_EASYSOCIAL_AT_LOCATION").done(function(){var c={BACKSPACE:8,COMMA:188,DELETE:46,DOWN:40,ENTER:13,ESCAPE:27,LEFT:37,RIGHT:39,SPACE:32,TAB:9,UP:38};
EasySocial.Controller("Locations",{defaultOptions:{view:{suggestion:"site/location/story.suggestion"},map:{lat:0,lng:0},"{textField}":"[data-location-textField]","{detectLocationButton}":"[data-detect-location-button]","{suggestions}":"[data-location-suggestions]","{suggestion}":"[data-story-location-suggestion]","{mapPreview}":"[data-location-map]","{latitude}":"[data-location-lat]","{longitude}":"[data-location-lng]"}},function(b){return{init:function(){navigator.geolocation&&b.detectLocationButton().show(),EasySocial.require().library("gmaps").done(function(){b.textField().removeAttr("disabled")
})},navigate:function(a,c){var d=b.mapPreview(),e=b.map;void 0===e&&(e=b.map=d.show().gmaps(b.options.map)),e.setCenter(a,c),e.removeMarkers(),e.addMarker({lat:a,lng:c})},locations:{},lastQueryAddress:null,"{textField} keypress":function(d,e){switch(e.keyCode){case c.UP:var f=a(b.suggestion(".active").prev(b.suggestion.selector)[0]||b.suggestion(":last")[0]);b.suggestion().removeClass("active"),f.addClass("active").trigger("activate"),b.suggestions().scrollTo(f,{offset:-1*f.height()}),e.preventDefault();break;
case c.DOWN:var g=a(b.suggestion(".active").next(b.suggestion.selector)[0]||b.suggestion(":first")[0]);b.suggestion().removeClass("active"),g.addClass("active").trigger("activate"),b.suggestions().scrollTo(g,{offset:-1*g.height()}),e.preventDefault();break;case c.ENTER:var h=b.suggestion(".active"),i=h.data("location");b.set(i),b.suggestions().hide();break;case c.ESCAPE:}},"{textField} keyup":function(d,e){switch(e.keyCode){case c.UP:case c.DOWN:case c.ENTER:case c.ESCAPE:break;default:var f=a.trim(d.val());
if(""===f&&b.suggestions().hide(),f==b.lastQueryAddress)return;var g=b.locations[f];g?(b.suggest(g),b.lastQueryAddress=f):b.lookup(f)}},lookup:a._.debounce(function(a){EasySocial.ajax("site/controllers/location/suggestLocations",{address:a}).done(function(c){b.locations[a]=c,b.suggest(c),b.textField().focus()})},250),suggest:function(c){var d=b.suggestions();d.hide().empty(),a.each(c,function(a,c){b.view.suggestion({location:c}).data("location",c).appendTo(d)}),d.show()},"{suggestion} activate":function(a){var d=a.data("location");
b.navigate(d.latitude,d.longitude)},"{suggestion} mouseover":function(a){b.suggestion().removeClass("active"),a.addClass("active").trigger("activate")},"{suggestion} click":function(a){var d=a.data("location");b.set(d),b.suggestions().hide()},set:function(a){b.currentLocation=a;var c=a.name;b.textField().val(c),b.latitude().val(a.latitude),b.longitude().val(a.longitude),b.trigger("locationChange",[a])},"{detectLocationButton} click":function(){b.map;a.GMaps.geolocate({success:function(a){EasySocial.ajax("site/controllers/location/getLocations",{latitude:a.coords.latitude,longitude:a.coords.longitude}).done(function(a){b.suggest(a),b.textField().focus()
})},error:function(){},always:function(){}})},"{story} save":function(a,c,d){var e=b.currentLocation;e&&d.addData(b,{short_address:e.name,formatted_address:e.address,lat:e.latitude,lng:e.longitude})},"{story} clear":function(){b.unset()}}}),b.resolve()})}),EasySocial.module("pagination",function(){var a=this;EasySocial.require().done(function(b){EasySocial.Controller("Pagination",{defaultOptions:{"{pages}":".pageItem","{limitstart}":"#limitstart","{previousItem}":".previousItem","{nextItem}":".nextItem"}},function(a){return{init:function(){a.pages().implement(EasySocial.Controller.Pagination.Page,{pagination:a})
...
})}}}),b.resolve()})}),EasySocial.module("site/events/item",function(){var b=this;EasySocial.template("info/item",'<li data-sidebar-item><a class="ml-20" href="[%= url %]" title="[%= title %]" data-info-item data-info-index="[%= index %]"><i class="ies-info ies-small mr-5"></i> [%= title %]</a></li>'),EasySocial.require().script("site/friends/suggest","site/events/guestState").view("site/loading/small").library("history").done(function(a){EasySocial.Controller("Events.Item",{defaultOptions:{id:null,"{filterStreamList}":"[data-filter-stream-list]","{sidebarItem}":"[data-sidebar-item]","{addFilter}":"[data-filter-add]","{editFilter}":"[data-filter-edit]","{filterStream}":"[data-filter-stream]","{filterApp}":"[data-filter-app]","{showAllFilters}":"[data-filter-showall]","{apps}":"[data-app-item]","{content}":"[data-content]","{saveHashtag}":"[data-hashtag-filter-save]","{invite}":"[data-action-invite]","{unpublish}":"[data-action-unpublish]","{delete}":"[data-action-delete]","{guestStateWrap}":"[data-guest-state-wrap]","{info}":"[data-info]","{infoItem}":"[data-info-item]","{menuItem}":"[data-dashboardSidebar-menu]",view:{infoItem:"info/item",loading:"site/loading/small"}}},function(b){return{init:function(){b.initGuestStates()
...
}),b.resolve()})}),EasySocial.module("site/groups/groups",function(){var b=this;EasySocial.require().view("site/loading/small").script("validate","field").done(function(a){EasySocial.Controller("Groups.Browser",{defaultOptions:{"{filters}":"[data-es-groups-filters] > li","{categories}":"[data-es-groups-categories] >li","{content}":"[data-es-groups-content]","{items}":"[data-groups-item]","{featured}":"[data-groups-featured-item]",view:{loader:"site/loading/small"}}},function(b){return{init:function(){b.filters().implement(EasySocial.Controller.Groups.Filter,{"{parent}":b}),b.categories().implement(EasySocial.Controller.Groups.Filter.Category,{"{parent}":b}),b.featured().implement(EasySocial.Controller.Groups.Browser.Item,{"{parent}":b}),b.items().implement(EasySocial.Controller.Groups.Browser.Item,{"{parent}":b})
},setActive:function(c){b.categories().removeClass("active"),b.filters().removeClass("active"),a(c).addClass("active"),b.content().html("&nbsp;"),b.element.addClass("loading")},setContent:function(a){b.element.removeClass("loading"),b.content().html(a),b.items().implement(EasySocial.Controller.Groups.Browser.Item,{"{parent}":b})}}}),EasySocial.Controller("Groups.Browser.Item",{defaultOptions:{"{joinGroup}":"[data-groups-item-join]","{leaveGroup}":"[data-groups-leave]","{respond}":"[data-groups-item-respond]","{setFeatured}":"[data-groups-item-set-featured]","{removeFeatured}":"[data-groups-item-remove-featured]","{deleteGroup}":"[data-groups-item-delete]","{unpublishGroup}":"[data-groups-item-unpublish]","{footer}":"[data-groups-item-footer]"}},function(a,b,c){return{init:function(){b.id=a.element.data("id"),b.type=a.element.data("type")
...
}).done(function(){b.profileForm().submit()}),!1}}}),EasySocial.Controller("Groups.Filter",{defaultOptions:{}},function(b){return{init:function(){},"{self} click":function(c,d){d.preventDefault(),b.parent.setActive(c),a(c).find("a").route(),EasySocial.ajax("site/controllers/groups/getGroups",{filter:b.element.data("es-groups-filters-type")}).done(function(a){b.parent.setContent(a)})}}}),EasySocial.Controller("Groups.Filter.Category",{defaultOptions:{id:null}},function(b){return{init:function(){b.options.id=b.element.data("es-groups-category-id")
...
return d}}}}),b.textboxlistLib=b.customTextInput().textboxlist("controller")},getTaggedUsers:function(){var c=[],d=b.customInputItem();return d.length>0&&a.each(d,function(b,d){c.push(a(d).data("id"))}),c},"{customTextInput} addItem":function(a,c,d){var e=b.hiddenCustom().val(),f="";if(""==e)f=d.id;else{var g=e.split(",");g.push(d.id),f=g.join(",")}b.hiddenCustom().val(f)},"{customTextInput} removeItem":function(a,c,d){for(var e=b.hiddenCustom().val(),f="",g=[],h=e.split(","),i=0;i<h.length;i++)h[i]!=d.id&&g.push(h[i]);
f=g.length<=0?"":g.join(","),b.hiddenCustom().val(f)},"{customEditBtn} click":function(){b.customForm().toggle()},"{selection} change":function(a){var c=a.val();"custom"==c?(b.customForm().show(),b.customEditBtn().show()):(b.customForm().hide(),b.customEditBtn().hide())},"{customHideBtn} click":function(){b.customForm().hide(),b.customEditBtn().show(),b.textboxlistLib.autocomplete.hide()}}}),b.resolve()})}),EasySocial.module("site/profile/profile",function(){var b=this;EasySocial.template("info/item",'<li data-profile-apps-item data-layout="custom"><a class="ml-20" href="[%= url %]" title="[%= title %]" data-info-item data-info-index="[%= index %]"><i class="ies-info ies-small mr-5"></i> [%= title %]</a></li>'),EasySocial.require().script("site/profile/header","site/profile/feeds").library("history").done(function(a){EasySocial.Controller("Profile",{defaultOptions:{id:null,"{header}":"[data-profile-header]","{feeds}":"[data-profile-feeds]","{app}":"[data-profile-apps-item]","{action}":"[data-profile-apps-menu]","{showAllFilters}":"[data-app-filters-showall]","{appFilters}":"[data-sidebar-app-filter]","{contents}":"[data-profile-real-content]","{sidebar}":"[data-sidebar]","{sidebarToggle}":"[data-sidebar-toggle]","{info}":"[data-info]","{infoItem}":"[data-info-item]",view:{infoItem:"info/item"}}},function(b){return{init:function(){b.options.id=b.element.data("id"),b.header().implement(EasySocial.Controller.Profile.Header,{"{parent}":b}),b.feedsController=b.feeds().addController(EasySocial.Controller.Profile.Feeds,{"{parent}":b}),a(function(){b.setLayout()
...
}}).done(function(a){b.updateContent(a)}).fail(function(a){return a})),void 0)},"{showAllFilters} click":function(c){a(c).hide(),b.appFilters().removeClass("hide")},updateContent:function(a){b.element.removeClass("loading"),b.contents().html(a)},updatingContents:function(){b.element.addClass("loading")},loading:function(){b.contents().html(""),b.element.addClass("loading")},"{info} click":function(c,d){d.preventDefault(),c.route(),b.loading();var e=c.data("loaded");return e?(b.infoItem().eq(0).trigger("click"),void 0):(EasySocial.ajax("site/controllers/profile/initInfo",{id:b.options.id}).done(function(d){c.data("loaded",1);
var e=c.parent("[data-profile-apps-item]");a.each(d.reverse(),function(a,c){c.hide||e.after(b.view.infoItem({url:c.url,title:c.title,index:c.index})),c.html&&(b.updateContent(c.html),b.contents().find("[data-field]").trigger("onShow"))});var f=b.infoItem().eq(0).parent("[data-profile-apps-item]");b.app().removeClass("active"),f.addClass("active"),a(document).prop("title",b.infoItem().eq(0).attr("title"))}),void 0)},"{infoItem} click":function(a,c){c.preventDefault(),a.route(),b.loading();var d=a.data("info-index");
...
},250))},"{loadmorebutton} click":function(){b.loadMore()},loadMore:function(){var c=a("[data-search-query]").val(),d=a("[data-sidebar-menu].active").data("type"),e=b.pagination().data("last-limit"),f=b.pagination().data("last-type");return"-1"==e?(b.loadmorebutton().hide(),void 0):(b.loading=!0,EasySocial.ajax("site/controllers/search/getItems",{next_limit:e,last_type:f,type:d,q:c,loadmore:"1"},{beforeSend:function(){b.pagination().html(b.view.loadingContent())}}).done(function(a,c,d){b.pagination().data("last-limit",d),b.pagination().data("last-type",c),b.pagination().before(a),b.item().implement(EasySocial.Controller.Search.Item),"-1"==d?b.pagination().html(""):b.pagination().html(b.view.loadmoreContent())
...
},loadMoreGuest:function(){var c=a.paginationGuest(),d=c.data("nextlimit"),e=a.options.source;d&&(a.loading=!0,c.html(a.view.loadingContent({content:""})),EasySocial.ajax("site/controllers/stream/loadmoreGuest",{startlimit:d,view:e}).done(function(d,e){c.data({nextlimit:e});var d=b.buildHTML(d);d.insertBefore(c).filter(a.item.selector).addController("EasySocial.Controller.Stream.Item");try{MathJax&&MathJax.Hub.Queue(["Typeset",MathJax.Hub])}catch(f){}""==e?c.html(""):c.html(a.view.loadmoreContent())}).fail(function(a){return a
}).always(function(){a.loading=!1}))},"{paginationCluster} click":function(){a.loadMore(!0)},"{pagination} click":function(){a.loadMore()},loadMore:function(c){var d=b("[data-dashboardSidebar-menu].active"),e=d.data("type"),f=d.data("id");tag=d.data("tag"),fid=d.data("fid");var g=c===!0?a.paginationCluster():a.pagination(),h=g.data("nextlimit"),i=g.data("context"),j=a.options.source;h&&(void 0==e&&void 0==f&&"profile"==a.options.source&&(e="me",f=a.options.sourceId),a.loading=!0,g.html(a.view.loadingContent({content:""})),EasySocial.ajax("site/controllers/stream/loadmore",{id:f,type:e,startlimit:h,view:j,tag:tag,filterId:fid,context:i,iscluster:c}).done(function(c,d){g.data({nextlimit:d});
...
EasySocial.require().view("apps/user/links/story/attachment.item").done(function(){EasySocial.Controller("Story.Links",{defaultOptions:{view:{linkItem:"apps/user/links/story/attachment.item"},urlParser:/(^|\s)(https?:\/\/)?(([a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.([a-z]{2,6}))|(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]))(:[0-9]{1,5})?(\/.*)?/gi,"{linkForm}":"[data-story-link-form]","{linkInput}":"[data-story-link-input]","{linkContent}":"[data-story-link-content]","{linkItem}":"[data-story-link-item]","{linkTitle}":"[data-story-link-title]","{linkDescription}":"[data-story-link-description]","{linkImages}":"[data-story-link-images]","{linkImage}":"[data-story-link-image]","{imageWrapper}":"[data-story-link-image-wrapper]","{linkVideo}":"[data-story-link-video]","{titleTextfield}":"[data-story-link-title-textfield]","{descriptionTextfield}":"[data-story-link-description-textfield]","{panelButton}":"[data-story-link-panel-button]","{attachButton}":"[data-story-link-attach-button]","{removeButton}":"[data-story-link-remove-button]","{removeThumbnail}":"[data-story-link-remove-image]"}},function(b){return{init:function(){b.linkInput().placeholder()
...
})},"{uploadItemPreview} click":function(b){var c=a(b).data("uri"),d=a(b).data("title");a.dialog({title:d,content:a.Image.get(c)})}}})}),b.resolve()})});