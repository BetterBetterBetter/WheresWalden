				...
				<div class="col-md-9">
					...
					<div class="discussion-replies<?php echo !$replies ? ' is-empty' : '';?>" data-replies-wrapper>
						<div class="clearfix">
							...
							<a href="<?php echo FRoute::apps( array( 'layout' => 'canvas' , 'customView' => 'item' , 'uid' => $group->getAlias() , 'type' => SOCIAL_TYPE_GROUP , 'id' => $app->getAlias() , 'discussionId' => $discussion->id ) , false );?>#reply" class="pull-right btn btn-es-success btn-small mt-5 btn-post-reply"><?php echo JText::_( 'APP_GROUP_DISCUSSIONS_POST_REPLY' ); ?> <i class="ies-share-2 ies-small"></i></a>
							<?php }?>
						</div>
						<hr />
						...
						<ul class="fd-reset-list replies" data-reply-list>
							<?php foreach( $replies as $reply ){ ?>
								<?php echo $this->loadTemplate( 'apps/group/discussions/canvas/item.reply' , array( 'reply' => $reply , 'answer' => $answer, 'group' => $group, 'question' => $discussion) ); ?>
							<?php } ?>
						</ul>
				...
				<div class="col-md-3 discussion-meta">
					...
					<div class="mt-5">
						<a href="<?php echo FRoute::apps( array( 'layout' => 'canvas' , 'uid' => $group->getAlias() , 'type' => SOCIAL_TYPE_GROUP , 'id' => $app->getAlias() , 'customView' => 'create' ) );?>"
							class="btn btn-es-primary btn-create"><i class="ies-pencil"></i>&nbsp; <?php echo JText::_( 'APP_GROUP_DISCUSSIONS_CREATE_DISCUSSION' ); ?> &rarr;</a>
					</div>
					<?php } ?>
					...
					<div class="stats">
						...
						<ul class="list-unstyled">
							<li>
								<i class="ies-user"></i> <?php echo JText::sprintf( 'APP_GROUP_DISCUSSIONS_STARTED_BY' , $this->html( 'html.user' , $author->id ) ); ?>
							</li>
							<li>
								<i class="ies-calendar"></i> <?php echo FD::date( $discussion->created )->format( JText::_( 'DATE_FORMAT_LC1' ) );?>
							</li>
							<li>
								<i class="ies-eye"></i> <?php echo JText::sprintf( FD::string()->computeNoun( 'APP_GROUP_DISCUSSIONS_HITS' , $discussion->hits ) , $discussion->hits ); ?>
							</li>
							<li>
								<i class="ies-users"></i> <?php echo JText::sprintf( FD::string()->computeNoun( 'APP_GROUP_DISCUSSIONS_PARTICIPANTS' , count( $participants ) ) , count( $participants )); ?>
							</li>
							<li>
								<i class="ies-comments"></i> <?php echo JText::sprintf( FD::string()->computeNoun( 'APP_GROUP_DISCUSSIONS_TOTAL_REPLIES' , $discussion->total_replies ) , '<span data-reply-count>' . $discussion->total_replies . '</span>' ); ?>
							</li>

							<?php if( $answer ){ ?>
							<li>
								<i class="ies-fire"></i> <?php echo JText::sprintf( 'APP_GROUP_DISCUSSIONS_ANSWERED_BY' , '<a href="#reply-' . $answer->id . '">' . $answer->author->getName() . '</a>' ); ?>
							</li>
							<?php } ?>
						</ul>
					</div>
