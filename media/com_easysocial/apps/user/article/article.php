...
class SocialUserAppArticle extends SocialAppItem
{
	...
	public function onPrepareStream( SocialStreamItem &$item, $includePrivacy = true )
	{
		...
		if ($item->verb == 'create' && $params->get('stream_create', false)) {
			$this->prepareCreateArticleStream($item);
		}

		if ($item->verb == 'update' && $params->get('stream_update', false)) {
			$this->prepareUpdateArticleStream($item);
		}

		if ($item->verb == 'read' && $params->get('stream_read', false)) {
	...
	private function processContentLength($content)
	{
		...
		$params = $this->getParams();
		$contentLength = $params->get('stream_content_length', 250);

		// If there is no truncation necessary
		if (!$contentLength) {
			$base = JURI::base(true).'/';
		...
		// Real truncation happens here.
		$content = JString::substr($content, 0, $contentLength) . '... ';
