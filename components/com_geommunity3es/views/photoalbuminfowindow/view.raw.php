...
class geommunity3esViewPhotoalbuminfowindow extends JViewLegacy
{
	function display($tpl = null) 
	{
		...
		$this->infowindow  	= $this->get('Photoalbuminfowindow');
		...
		if($naming=='realname')
			...
			$this->naming = $infowindow->username;	
		$this->title			= $infowindow->albumname;	
		...
		$this->albumphotocount	= $infowindow->albumphotocount;
		$this->hits				= $infowindow->hits;
		$this->user_id			= $infowindow->user_id;
		$this->photoalbum_url = JRoute::_('index.php?option=com_easysocial&view=albums&id='.$this->id.':'.JFilterOutput::stringURLSafe($this->title).'&layout=item&uid='.$this->user_id.':'.$this->naming.'&type=user&Itemid='.$photo_itemid );
		parent::display($tpl);
	}
}